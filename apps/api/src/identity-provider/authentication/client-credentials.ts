...
export class ClientCredentialsProvider {
  ...
  // @todo: Implement access token cache for IdPs? This might save one request and therefore some milliseconds...
  ...
  async authenticate(
    ...
    if (!response.ok) {
      this.logger.error(
        `Failed to authenticate towards IdP: ${await response.text()}`,
      );
